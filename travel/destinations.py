...
destbp = Blueprint('destination', __name__)

@destbp.route('/destinations/<id>')
def show(id):
    destination = db.session.scalar(db.select(Destination).where(Destination.id==id))
    # create the comment form
    print(destination.image)
    form = CommentForm()    
    return render_template('show.html', destination=destination, form=form)


@destbp.route('/destinations/<id>/comment', methods=['GET', 'POST'])  
@login_required
def comment(id):  
    form = CommentForm()  
...
def create():
  ...
  return render_template('createDestination.html', form=form)
